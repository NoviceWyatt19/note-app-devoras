...
### 5.1 어두운 hue 배경(`-950`·`-900` 계열) — **신규 토큰을 만들지 않는다**

`levelColors`(`MindView.tsx:392-399`) 같은 곳이 `bg-teal-950/20` · `shadow-purple-950/10` 처럼
**거의 검은 hue 틴트**를 쓴다. 이걸 `bg-node1/20` 으로 바꾸면 **밝은 워시**가 되어 합성 RGB 가
**28~37 단위** 달라진다 — 육안으로 확실히 다르다. 그렇다고 `--node-N-bg` 를 새로 만들면 토큰이 다시 늘어난다.

**해법: 같은 토큰을 낮은 임의 알파로 쓴다.** 원본을 총 RGB 차 2~11 단위로 재현한다(실측):
...
`hover:` 변형과 `shadow-` 색상 유틸리티도 정상 생성된다.

> **매핑표 적용 시 변형 접두사를 빠뜨리지 말 것.** `border-l-`·`hover:`·`focus:`·`shadow-` 등이
> 붙어도 색 계열은 같으므로 같은 규칙을 적용한다(T3a 에서 `border-l-*` 7곳이 집계에서 누락됐던 사례).

### 5.2 노드 400/500 혼재 — **하나로 합친다**

한 뱃지가 `text-teal-400` + `bg-teal-500/20` + `border-teal-500/30` 처럼 **두 셰이드를 함께 쓴다.**
Tailwind 관용이지 설계된 구분이 아니다. `--node-N` 하나로 합친다.

**합성 차이 실측**: `/20` 에서 1~10 단위, `/30` 에서 1~14 단위. **감지 불가 범위다.**
(§4.8 의 8지점에 이 변경도 포함해 센다.)

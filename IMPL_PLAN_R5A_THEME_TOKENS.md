...
| `--accent-soft-rgb` | `129 140 248` (`#818cf8`) | `67 56 202` | 링크 |
| `--accent-subtle-rgb` | `165 180 252` (`#a5b4fc`) | `55 48 163` | 인라인 코드 |
...
| `--warning-rgb` | `245 158 11` (`#f59e0b`) | `180 83 9` | 경고 · dirty 표시 |
| `--info-rgb` | `147 197 253` (`#93c5fd`) | `37 99 235` | 정보 |
| `--highlight-rgb` | `253 224 71` (`#fde047`) | `113 63 18` | 하이라이트(`==text==`) |
...
| 레벨 | `--node-N-rgb` 다크 | **라이트** | `--node-N-tint-rgb` 다크 | **라이트** |
|---|---|---|---|---|
| 1 teal | `45 212 191` | **`17 94 89`** | `20 184 166` | **`13 148 136`** |
| 2 sky | `56 189 248` | **`7 89 133`** | `14 165 233` | **`2 132 199`** |
| 3 amber | `251 191 36` | **`146 64 14`** | `245 158 11` | **`217 119 6`** |
| 4 pink | `244 114 182` | **`157 23 77`** | `236 72 153` | **`219 39 119`** |
| 5 purple | `192 132 252` | **`107 33 168`** | `168 85 247` | **`147 51 234`** |

> **라이트 값 근거** (뱃지 = `tint/20` 배경 위의 `hue` 텍스트):
> hue 를 **Tailwind 800** 으로 두면 텍스트 대비 **5.28~5.92** ✅. 700 은 3.74~4.74 로
> teal·amber·pink 가 미달이라 쓸 수 없다. tint 를 **600** 으로 두면 뱃지 배경이 패널과
> **1.23~1.34** 로 구분된다.
>
> ⚠️ **라이트에서 hue/tint 의 밝기 순서가 뒤집힌다** — 다크는 hue(400)가 tint(500)보다 **밝고**,
> 라이트는 hue(800)가 tint(600)보다 **어둡다.** indigo 사다리(§4.9)와 같은 반전이며,
> **어두운 배경에서는 밝은 글자, 밝은 배경에서는 어두운 글자**라는 같은 원리다.
...
### 4.9 라이트 팔레트 대비 검산 — **T6 착수 전에 이미 수행했다**

초안 라이트 값을 WCAG 명도대비로 전수 검산했더니 **3개가 실패했다.** 표에 반영된 값은 정정 후다.

| 토큰 | 초안 | 실제 배경 위 대비 | 정정 | 정정 후 |
|---|---|---|---|---|
| `--accent-subtle` (인라인 코드) | `129 140 248` | **2.49** ❌ | `55 48 163`(indigo-800) | **8.31** ✅ |
| `--highlight` (`==text==`) | `202 138 4` | **2.46** ❌ | `113 63 18`(yellow-900) | **7.26** ✅ |
| `--accent-soft` (링크) | `99 102 241` | 4.47 ⚠️ | `67 56 202`(indigo-700) | **7.90** ✅ |

> **왜 놓쳤나**: 초안은 다크 값을 「밝기 반전」한 것이었다. 그런데 **인라인 코드와 하이라이트는
> 배경도 함께 밝아진다** — 전경만 어둡게 하면 되는 게 아니라 **전경·배경 쌍으로 봐야** 한다.
> `surface-base` 기준으로만 재면 2.94·2.98 로 「경계」처럼 보이지만, **실제 배경(강조 12% 틴트,
> 하이라이트 18% 틴트) 위에서는 2.49·2.46 으로 명백한 실패**다.
>
> **라이트 모드에서 indigo 사다리는 방향이 뒤집힌다** — 다크는 「밝을수록 약함」(accent → soft → subtle),
> 라이트는 **「어두울수록 약함」**(indigo-600 → 700 → 800). 세 값을 함께 옮겨야 서로 구분이 남는다.

**남은 ⚠️ 2건은 의도적으로 통과시킨다:**

| 항목 | 대비 | 사유 |
|---|---|---|
| `--warning` 라이트 (dirty 점) | **4.58** | 초안 `217 119 6` 은 패널 위 **2.91 로 3:1 미달**이었다 → `180 83 9`(amber-700)로 정정 |
| `--text-faint` (다크 4.05 / 라이트 panel 위 3.86) | 3.8~4.1 | 비활성·취소선 전용. **본문에 쓰지 않는다** — T6 에서 이 용도가 지켜지는지만 확인 |

**다크는 전 항목 통과다**(텍스트 4.05~17.61). 이번 검산에서 실패한 것은 전부 라이트 초안값이다.
...
1. `index.css` 에 `:root[data-theme="light"] { … }` 블록으로 §4 의 라이트 값 36개를 정의한다.
...
- **T3 범위에서는 `WorkspacePage.tsx:307` 의 `bg-amber-500` 을 바꾸지 않았다.** §5 매핑표에서 제외했다.
- **T6 에서는 바꾼다 — 확정.** 실측: `#f59e0b` 는 라이트 패널 위 대비 **1.96 으로 비텍스트 3:1 기준 미달**이다.
  라이트 모드에서 dirty 점이 배경에 묻힌다. **`bg-warning` 으로 바꾸고 하네스 단언도 함께 갱신**한다
  (`save_dirty_indicator_harness.tsx` `:66`·`:80`·`:106` 의 `.bg-amber-500` → `.bg-warning`).
  **하네스와 코드를 같은 커밋에** 넣는다 — 나뉘면 그 사이 커밋에서 T1.5 가 깨진다.

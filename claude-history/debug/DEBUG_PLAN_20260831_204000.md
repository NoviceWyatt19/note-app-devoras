...
## 5.1 착지 커밋 — ⚠️ 커밋 이력 정정 (2026-08-31)

> **이 수정의 코드는 `fix(0.8.49)` 커밋으로 존재하지 않는다.** 찾을 때 헷갈리지 않도록 기록한다.

| 무엇 | 어디에 |
|---|---|
| 코드 전체(`focusToken`·`caretCoordinator.ts`·`caret_stability_harness.ts`·`BlockEditor.tsx` 조정자·**0.8.48 → 0.8.49 버전업**) | **`f48b88c`** — 제목은 `chore(0.8.48): 로드맵에 퀵 캡처 … 추가` |
| 문서(DoD·VERIFY_BY_HUMAN §11) | `5f980d0` |

**원인**: 공유 워킹 트리에서 계획 세션이 `git add function_roadmap.md` 로 **경로를 명시해 스테이징한 뒤 커밋**했으나, 그 시점 인덱스에는 구현 세션이 이미 `git add` 해둔 코드 변경이 함께 올라와 있었다. **`git commit` 은 스테이징한 경로가 아니라 인덱스 전체를 커밋한다.** 그 결과 코드와 버전업이 문서 커밋에 흡수됐고, 커밋 메시지의 "문서 전용이므로 버전을 올리지 않았다"는 내용과 실제가 어긋나게 됐다.

**정정 방식**: 이미 여러 세션이 이 이력 위에서 작업한 뒤라 **히스토리 재작성 대신 이 기록을 남기는 것으로 갈음한다**(사용자 결정). 코드 자체는 정상 착지했고 게이트도 통과했으므로 기능상 손실은 없다.

**재발 방지**: `.agents/AGENTS.md` 「공유 워킹 트리에서의 커밋」 참조.

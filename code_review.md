...
| **B2** 보안 | BUG-01 (P0-1) | ❌ 미해결 | ✅ **코드 종결 (v0.8.28~0.8.35)** — L1·L4-deny·L3·L2·L4-scope 전부 착지. 사람 손 실기 검증만 잔여 | 아래 「B2 — L1~L4-scope 구현」 · [`project/VERIFY_BY_HUMAN.md`](project/VERIFY_BY_HUMAN.md) |
| **B3** 입력 안정성 | BUG-08·09·10·11 | 미착수 | ✅ **종결 (v0.8.33~0.8.34)** — 4건 중 2건(08·10)은 착수 전 이미 구조적 소멸, 나머지 2건(09·11) 수정 | 아래 「Step 1~6 사이클」 |
| **B4** 구조 | REF-01·02 | 미착수 | ◻ **미착수 (Step 7)** — 선행 게이트(A6 판정)는 통과 | [`project/DEBUG_PLAN.md`](project/DEBUG_PLAN.md) §5 |
| **A6/A7 구조 결함** | ARCHITECTURE_FINDINGS | (리뷰 범위 밖) | ✅ **A7 종결 (v0.8.39~0.8.41)** / 🟡 **A6 판정 완료, 후속 티켓 발생 (v0.8.43)** | 아래 「Step 1~6 사이클」 |
...
- **✅ 해결 (L4-scope, v0.8.35)** — PoC 결과 `Scope::allow_directory()` 동적 허용이 성립함을 확인하고 `allow: ["**"]` 를 `allow: []` + `devoras_set_workspace_root` 런타임 동적 허용으로 전환. `fs:allow-home-{read,write}-recursive` 2건도 함께 제거.
  - **구조적 한계 명시**: Tauri `Scope` 의 허용 패턴은 **append-only** 다(제거 API 없음). `forbid_directory()` 는 "이후 항상 거부"라 워크스페이스 재오픈을 영구히 막으므로 쓸 수 없다. 따라서 **한 세션에서 연 모든 워크스페이스 루트의 합집합**이 허용 상태로 남는다. `"**"` 대비로는 압도적으로 좁지만 "현재 워크스페이스만"은 아니다.
  - **회귀 1건과 그 수정**: home 재귀 권한 제거가 **워크스페이스 밖 이미지 드래그앤드롭**을 깨뜨렸다. `devoras_read_dropped_file` Rust 커맨드로 우회 수정했으나, **최초 구현은 무검증 `std::fs::read(path)` 였다** — 페이지 JS 가 임의 절대경로를 읽을 수 있어 L3·L4 를 통째로 우회하는, 원래 막으려던 것보다 넓은 읽기 원시타입이었다. `DroppedPaths` 상태 + OS 드롭 이벤트 대조(1회용 소비) 방식으로 수정 완료. **교훈: scope 축소가 만든 회귀를 scope 우회로 메우면 축소가 무의미해진다.**

## Step 1~6 사이클 — 구조 결함 해소 (v0.8.32~0.8.43)

> 본 리뷰(2026-08-26)의 P0~P2 카탈로그 **밖**에서 도출된 구조 결함 A1~A7 의 해소 기록.
> 원 분석은 [`ARCHITECTURE_FINDINGS.md`](ARCHITECTURE_FINDINGS.md), 실행 사양은 [`project/DEBUG_PLAN.md`](project/DEBUG_PLAN.md) 및 그 아카이브.

| Step | 대상 | 결과 |
|---|---|---|
| 1 | B2 잔여 (L4-scope) | ✅ 코드 종결 — 위 「B2」 절 |
| 2 | B3 입력 안정성 (P1-5·P1-7) | ✅ 종결 (v0.8.33~0.8.34) |
| 3 | A5 중간 불변식 | ✅ R3 착지. R10 은 기존 적용분 |
| 4 | **A7 헤딩 정체성** | ✅ 종결 (v0.8.39~0.8.41) |
| 5 | **A1/A2 조정자 일원화 + 최소 diff** | ✅ 종결 (v0.8.42) |
| 6 | **A6 마운트 비용 판정 게이트** | ✅ 판정 완료 (v0.8.43) — **후속 티켓 발생** |
| 7 | B4 구조 개편 | ◻ 미착수 |

### A7 — 헤딩 정체성: 진단이 한 번 틀렸다가 정정된 사례

**최초 진단**: "헤딩 블록의 id 가 라벨에서 파생되므로 위치 기반으로 바꾼다."
**실제**: `EditorBlock.id` 는 **애초에 `generateId()` 로 만든 불투명 토큰**이었다(`store.ts:25`). 라벨 파생값은 id 가 아니라 **재파싱 시 old/new 블록을 대응시키는 매칭 키**였다. 즉 결함은 "id 가 내용에서 파생된다"가 아니라 **"제목을 고치면 매칭이 실패해 새 id 가 발급된다"** 였다.

이 정정이 중요한 이유 — 최초 진단대로 **순수 위치 기반 키로 갔다면 재정렬이 대신 깨졌다.** `reorderBlocks` 는 `ReadView.tsx:360` 의 드래그앤드롭이 호출하는 **실사용 기능**이므로, 드문 버그를 고치려다 살아 있는 기능에 회귀를 넣는 교환이 될 뻔했다.

**착지한 설계 — 2-패스 매칭** (`setBlocksFromContent`):

| | 매칭 근거 | 살아남는 조작 |
|---|---|---|
| Pass 1 | 콘텐츠 키(라벨 + 조상 경로) | **재정렬** — 라벨이 그대로면 어디로 옮겨져도 다시 잡힌다 |
| Pass 2 | pass 1 미매칭분을 (부모, 레벨) 그룹 안에서 대응 — 콘텐츠 동일 우선, 그다음 위치 | **제목 편집** — 구조가 그대로면 편집된 블록만 남으므로 안정적으로 짝지어진다 |

두 번의 후속 수정이 있었고 **둘 다 최초 구현이 통과시킨 테스트 밖에서 발견됐다**:

1. **부모 리네임 시 자식 미보존** (`ff3912d`) — 부모 제목을 고치면 자손 헤딩의 조상 경로가 바뀌어 pass 1·2 를 모두 놓쳤다. 부모 키 변환 맵으로 해소(문서 순서상 부모가 항상 먼저 확정되므로 단일 전방 패스로 충분).
2. **부모 리네임 + 자식 재정렬 동시 발생 시 id 스왑** (`2572292`) — 자식들이 pass 2 의 위치 폴백으로 떨어지는데 위치가 서로 바뀌어 **id 가 교환**됐다. 리마운트보다 나쁜 결과다(남의 undo 히스토리를 물려받는다). 그룹 내 **콘텐츠 동일 우선 매칭**을 앞에 두어 해소.

회귀 가드: `block_key_stability_harness.ts` K1~K9 (10/10).

> **부수 발견 (기록 가치 있음)**: 블록의 직렬화 형태가 **문서 내 위치에 따라 달라진다** — 마지막 블록만 후행 개행을 갖지 않는다(join 순서 아티팩트). 콘텐츠 동등 비교를 넣을 때 이 비대칭이 먼저 걸렸다. 앞으로 블록 콘텐츠에 대해 **해시·동등 비교를 도입하는 모든 지점**이 같은 함정을 밟는다 — 특히 P2-3 계열의 캐시 키와 Step 6 후속의 메모이제이션.

### A1/A2 — 조정자 일원화 + 최소 diff (v0.8.42)

`BlockEditor.tsx` 의 캐럿 복원(`useLayoutEffect`)과 내용 동기화(passive `useEffect`)가 **서로 다른 두 이펙트**였고, 정확성이 React 의 실행 순서(layout 이 passive 보다 먼저)에 걸려 있었다 — 코드 어디에도 명시되지 않고 주석으로만 방어되던 제약이다. 이를 **단일 조정자**로 통합하고, 평상시 동기화 경로였던 **문서 전체 치환**을 `computeMinimalChange`(공통 접두/접미 트림) 기반 최소 범위 치환으로 교체했다. 그 결과 커서·undo 입도·데코레이션 위치가 자동으로 보존된다.

부수 수확: **IME 이중 가드가 캐럿 복원 경로에는 빠져 있었다**는 것이 통합 과정에서 드러나 함께 메워졌다. 회귀 가드: `minimal_diff_harness.ts` D1~D8, `test:ime` 5/5 유지.

### A6 — 마운트 비용: 판정은 "세 번째 갈래"였다

계획은 두 갈래를 예상했다(레이아웃 스래싱이라 배치로 선형 회복 / `EditorView` 생성에 내재라 아키텍처 교체). **측정 결과는 어느 쪽도 아니었다.**

| 조건 | N=200 | 성장 지수 |
|---|---|---|
| 순수 CodeMirror 코어 | 35.7 ms | 선형 |
| Devoras 전체 (데코레이터 포함) | 1,363 ms | ≈1.97 |
| `markdownDecorationPlugin` 단독 / `codeBlockInteractionPlugin` 단독 | — | 각 ≈1.2 |
| `display:none` 격리(레이아웃 배제) | — | 1.79 — **여전히 초선형** |

- **아키텍처는 무죄.** 「블록당 CodeMirror 인스턴스」 설계 자체는 선형이고 38배 싸다. → [`DEBUG_STEP_PLAN.md`](DEBUG_STEP_PLAN.md) §0.1 **재개봉 불필요**, Step 7 은 현행 전제로 진행.
- **배치·`content-visibility`·측정 지연은 답이 아니다.** 전부 레이아웃/페인트 타이밍을 다루는 수단인데 `display:none` 격리에서도 초선형이 유지되므로 원인이 레이아웃이 아니다.
- **범인은 Devoras 자체 데코레이터 확장 계층** — 두 플러그인이 각각 ≈1.2 인데 **동시 사용 시 ≈1.97 로 치솟는 상호작용**이다.

**→ 신규 티켓 (A6-후속)**: 데코레이터 확장 계층의 인스턴스 수 축 초선형 상호작용. **N=200 / 300ms 예산은 이 수정 없이는 달성되지 않는다.** 기존 `protectedRegions.ts` 의 N² 부채(Open Q #5 — 문서 **길이** 축)와 같은 뿌리인지는 **미확인**이며, 데코레이터 중재 인프라([`implementation_plan.md`](implementation_plan.md) §S.4) 착수 시 함께 조사할 가치가 있다. 재현: `src/widgets/BlockEditor/__tests__/mount_cost_t2_harness.ts`.

> **가상화(E7)에 대한 함의 변경**: 원래 조건은 "선형 회복 후 재평가"였으나 **"데코레이터 계층 수정 후 재평가"** 로 바뀐다. 그 수정 없이 가상화만 넣으면 뷰포트 안의 블록들끼리 여전히 같은 상호작용 비용을 치른다 — 가상화는 "동시에 몇 개가 사는가"만 줄이고 "여럿일 때 상호작용 비용이 있는가"는 고치지 않는다.
...
## P1-4. 비헤딩 블록이 첫 줄 원문을 키로 사용 → 블록 간 ID 탈취 **[해결 (G0, v0.8.11)]**
...
> **✅ 해결 (G0, v0.8.11)** — 실제 착지 형태는 `${parentId}-textblock-${index}` 로, 제안과 동일한 위치 기반 키다(`store.ts:60-65`). 회귀 가드: `block_key_stability_harness.ts` K1~K3.

---

## P1-5. 키 입력 1회당 `onUpdate`가 2회 호출 **[해결 (v0.8.33)]**
...
> **✅ 해결 (BUG-20260826-09, `dd52e32` v0.8.33)** — 제안대로 `domEventHandlers.input` 삭제. `pnpm test:ime` 5/5 수정 전후 동일.

---

## P1-6. 임의 블록 언마운트가 전역 활성 에디터 뷰를 null로 만듦 **[구조적 소멸]**
...
> **✅ 구조적 소멸** — 전역 싱글턴 `activeEditorView.ts` 자체가 `shared/lib/editorViewRegistry.ts`(패널 스코프 레지스트리)로 교체되어 이 결함의 전제가 사라졌다. `unregisterEditorView` 는 `caretHolder.get(paneId) === blockId` 일 때만 정리하므로 무관한 블록의 파괴가 포인터를 비우지 않는다. **본 항목의 `setActiveEditorView(null)` 코드는 현재 소스 트리에 존재하지 않는다.**

---

## P1-7. 병합 시 `#{1,4}`만 제거하며, 존재하지 않을 수 있는 ID로 포커스 복원 **[해결 (v0.8.34)]**
...
> **✅ 해결 (BUG-20260826-11, `2ec8a85` v0.8.34)** — 제안 그대로 착지. 같은 커밋에 R3 중간 불변식(`store.focusOffset === view.selection.main.head`, DEV 전용)을 함께 심어 이 계열의 다음 회귀가 최종 증상이 아니라 단언에서 잡히도록 했다.
>
> 본문의 "**P1-4와 직결**" 지적은 정확했다. 다만 P1-4(비헤딩) 해소 후에도 **헤딩 블록의 재생성 시 id 변화**라는 축이 남아 있었고, 그것이 A7 이다 — 아래 「Step 1~6 사이클」 참조.
...
> **✅ 해결 (v0.8.28)** — 권장대로 L1(DOMPurify) 커밋에서 블록당 재파싱+새니타이즈 캐시를 함께 넣었다.
...
## P2-5. `generateId`의 `Math.random` 의존 *(미해결 — **중요도 상승, P2→P1 재평가 권장**)*

**[파일 & 라인]** `block/store.ts:25`
**[조치 방안]** `crypto.randomUUID()` 사용. 여기서의 충돌은 React key와 포커스 타깃을 동시에 망가뜨리며 **P1-4와 증상이 구분되지 않습니다.**

> **⚠️ 2026-08-30 재평가 — 등급을 올려야 한다.** A7 수정(v0.8.39~0.8.41)으로 이 id 의 **역할이 바뀌었다.**
> 이전에는 재생성마다 새로 발급되는 소모품이었으나, 지금은 **2-패스 매칭이 문서 재파싱을 가로질러 운반하는 정체성 그 자체**다.
> 따라서 충돌의 결과가 "리마운트"에서 **"오매칭"** 으로 악화된다 — 충돌한 블록이 남의 `EditorView` 와 undo 히스토리를 물려받는다.
> `substring(2,9)` 7자 base36(≈36^7)은 문서당 블록 수 기준으로는 넉넉하지만, **한 세션에서 열리는 모든 문서·모든 재생성에 걸쳐 누적**되므로 여유는 계산해 본 적이 없다. `crypto.randomUUID()` 로의 교체 비용이 사실상 0이라는 점을 감안하면 방치할 이유가 없다.
...
## 📊 조치 우선순위 요약 **(2026-08-30 재정렬, v0.8.43 기준)**

> 이전 판은 **v0.8.5 기준 스냅샷**이었고 13개 항목 중 9개가 그 뒤 해소됐다. 아래는 **현재 열려 있는 것만** 남긴 재정렬이다. 해소분은 각 항목 절의 ✅ 표기와 위 「Step 1~6 사이클」 참조.

| 순위 | 항목 | 유형 | 영향도 | 난이도 |
|---|---|---|---|---|
| 1 | **P0-3** 분할 패널 표시 불일치 (Stage A-2) — Step 7-A | 🟠 UX/구조 | 중간 | 중간 |
| 2 | **A6-후속** 데코레이터 계층 초선형 상호작용 | 🟢 성능 | **높음** (N=200 예산 미달의 유일 원인) | 미확정 |
| 3 | **P1-9** `.cm-line *` 위젯 타이포그래피 붕괴 (B5, 독립) | 🟡 표시 | 중간 | 쉬움 |
| 4 | **P2-5** `generateId` 충돌 — **P2→P1 재평가 권장** | 🟡 버그 | 중간 | 쉬움 |
| 5 | **P1-3** 워크스페이스 전환 시 blockStore 잔존 — Step 7-B | 🟡 메모리 | 중간 | 쉬움 |
| 6 | **P0-3 Stage B** 탭 스코프 스토어 — Step 7-C | 🟠 구조 | 높음 | 높음 |
| 7 | B2 실기 검증 잔여 (사람 손) | 🔒 보안 | 미확정 | — |
| 8 | P2-1 · P2-2 렌더링 최적화 | 🟢 성능 | 중간 | 중간 |
| 9 | P2-4 · P2-6 ~ P2-8 코드 품질 | 🟢 품질 | 낮음 | 쉬움 |

**해소됨** — P0-1(B2 코드 종결) · P0-5 · P1-1 · P1-2 · P1-4 · P1-5 · P1-6 · P1-7 · P2-3, 그리고 리뷰 범위 밖이던 A1·A2·A5·A7.

### 권장 진행 순서

1. **Step 7 을 이어서 (진행 중인 배치의 잔여)** — 7-A(분할 패널 표시) → 7-B(선택적) → 7-C(탭 스코프 스토어). A6 판정이 아키텍처를 무죄로 결론냈으므로 **설계 전제 변경 없이 그대로 진행 가능**하다. 7-C 는 `advanced_rendering_optimization` Phase 1(상태 스냅샷)과 **동일 지점**이므로 반드시 함께 처리해 스냅샷 인터페이스를 두 번 설계하지 않는다.
2. **A6-후속 (데코레이터 계층)** — Step 7 과 독립이며 파일도 겹치지 않는다. **N=200 / 300ms 예산 미달의 유일한 원인**이므로 성능 목표를 되살리려면 이것부터다. 가상화(E7)는 이 수정 **뒤에** 재평가한다.
3. **커서 유실 계열의 마지막 조각 — P2-5.** P1-4·P1-7·A7 이 해소되면서 이 계열은 대부분 닫혔으나, `generateId` 충돌만 남았고 A7 이후 **오매칭으로 악화**됐다(§P2-5 재평가 참조). 교체 비용이 사실상 0이다.
4. **언제든 착수 가능한 독립 항목 — P1-9.** 다른 항목과 의존이 없고 사용자에게 즉시 보이는 표시 결함이다.
5. **사람 손이 필요한 것 — B2 실기 검증.** [`project/VERIFY_BY_HUMAN.md`](project/VERIFY_BY_HUMAN.md) 참조. 코드 작업이 아니므로 위 순서와 병행 가능하다.

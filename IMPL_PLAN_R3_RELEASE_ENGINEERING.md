# IMPL_PLAN — R3 출시 엔지니어링

> ## 🛑 배포 부분 종료 (2026-09-14, 사용자 확언)
>
> > 「**당장 배포할 계획은 없어.** 그러니 **github actions, release 같은 배포 기능, app store 등록 등의
> > 배포 활동은 고려하지 마.**」
>
> **R3 의 배포 부분은 스코프 밖이다.** 아래 §4(E1' 릴리스 스크립트)·Homebrew·서명·공증·업데이터·
> 태그 발행은 **전부 수행하지 않는다.** `scripts/release.mjs` 는 커밋돼 있으나 **아무도 실행하지 않는다.**
>
> ### 살아남은 것 — **로컬 git hook**
>
> §3(E2')의 훅은 **배포가 아니라 개발 품질 게이트**다. PM 이 R3 를 지시한 원래 목적
> (「하네스가 사람 기억에 의존하는 상태를 끊는다」)이 **그것 하나로 달성됐다.**
>
> | | 상태 |
> |---|---|
> | `pre-commit` — 버전 4파일 동기 (33ms) | ✅ 착지·실측 검증 |
> | `pre-push` — `tsc` + `t1` + `t15` (약 26초) | ✅ 착지·실측 검증 |
> | `check-version-sync.mjs` | ✅ 훅이 재사용 |
> | `.github/workflows/` | ✅ 삭제됨 |
> | 릴리스 발행 · 태그 · Homebrew · 서명 | 🛑 **하지 않음** |
>
> ### 이 문서를 읽는 다음 세션에게
>
> **§4 이하의 릴리스 설계는 기록으로만 둔다.** 나중에 배포가 필요해지면 그때 되살리면 되고,
> 그 시점에는 `check-version-sync.mjs` 와 훅이 그대로 재사용된다.
> **지금 그 절을 실행 지시로 읽지 말 것.**

...
| 5 | ⏳ **Step 7-C** 탭 스코프 스토어(REF-20260831-01) — C-1~C-4 완료, C-5(문서화)만 남음 | C-1: 동작 변화 0 · C-2: `test:paneownership` 5/5 + `test:ime` 5/5 · C-3: Provider 최초 실마운트 + `test:activetab` 12/12 신설 · C-4 ⚠️되돌리기 지점: 전역 싱글턴 제거, `ownerTabId`/`useBlockStore` grep 0건 확인, T1 76체크 전량 통과 — 실기는 전부 VERIFY_BY_HUMAN.md §8~§10 로 이월 | C-1 `d8f5545` · C-2 `bd3fabf` · C-3 `9a7c82c` · C-4 `ac2354c` |
...
- ✅ **C-4 — 전역 싱글턴 제거 + `ownerTabId` 소멸** ⚠️ 되돌리기 지점 (`ac2354c`, 0.8.48): 사용자가 명시적으로 착수 확인(자동 진행 아님). D-4 mandate 대로 재-grep 후 착수 — `ownerTabId`/`useBlockStore` 참조가 정말 0인지(주석 제외) 직접 확인했다. `useBlockStore` 싱글턴과 `useDocumentStore` 의 `rawContent`/`nodes`/`isDirty`/`viewMode` 필드를 소스 트리에서 완전히 제거. `openTab`/`_activateTabContent`/`_snapshotActiveTab`/`updateContentForTab`/`saveFile` 을 전역 필드 대신 `tabStoreRegistry` 로 재배선. 커밋 중 C-3 가 남긴 구멍 2건을 함께 발견·수정: (1) `saveFile` 의 spatialData 영구 저장이 전역 라이브 반영에 의존했는데 C-3 이후 끊겨 있었다 — 저장 시점에 탭 스토어의 `nodes` 에서 직접 재구성하도록 수정. (2) 마인드맵 노드 드래그가 탭 스코프 스토어의 isDirty 는 켜지만 탭 바 점(`TabItem.isDirty`)은 안 켰다 — `markTabDirty` 신설로 수정. 하네스 5종을 탭 스코프 스토어 + 레지스트리 기반으로 재작성(`__clearTabStoreRegistryForTests` 신설, 테스트 전용). `tsc`·`pnpm build`·`test:t1`(76 체크) 전부 통과.

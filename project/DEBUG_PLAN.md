...
**[상태]** ✅ 코드 완료(`entities/block/model/store.ts`, `shared/lib/headingId.ts` 무변경, `widgets/BlockEditor/ui/BlockEditor.tsx` G1 로그 추가). T1 `test:blockkey` K1~K7 전부 통과. 실기(T3) 확인 — undo 보존·실제 리마운트 0회 육안 확인 — 은 사람 손 필요, `VERIFY_BY_HUMAN.md` 로 이월.

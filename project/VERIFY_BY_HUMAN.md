...
- **⚠️ 미해결 관찰(2026-08-30, 사용자 실기 보고)**: 코드블록은 정상 렌더링되지만, **빌드 앱을 첫 실행하면 이미지가 렌더링되지 않는다.** 그런데 `pnpm tauri dev` 를 한 번 실행해 그쪽에서 이미지가 정상 렌더링된 뒤 다시 빌드 앱으로 돌아가면, 그때부터는 빌드 앱에서도 정상 렌더링된다(그 이후 드래그앤드롭·단축키도 정상 동작). 아직 근본 원인 미확정 — dev/build 가 별개 프로세스이고 asset scope 는 프로세스별 인메모리 상태라 "dev 에서 한 번 성공하면 build 에서도 된다"는 게 설명되지 않는다. **재확인 필요**: 이번 배치의 다른 두 수정(`fs:allow-app-write-recursive`, closeTab 동기화)이 우연히 해소했는지, 완전히 새로운 `$APPDATA`(예: 앱 삭제 후 재설치)에서도 재현되는지, 재현 시 빌드 앱에서 실패한 첫 이미지 요청의 정확한 `asset://` URL 과 콘솔/네트워크 탭의 상태 코드.

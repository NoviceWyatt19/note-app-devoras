...
| 위치 | 내용 | 상태 |
|---|---|---|
| `implementation_plan.md` §4A.2~4A.8 | **사양이 두 벌 존재**(`:1087~` / `:1611~`). 기계적 삭제 불가 — 상세는 아래 4.5 | 경고 삽입 + 상향 완료. **해소는 사용자 판단 대기** |
| `code_review.md` §2 | T1 하네스 "14종" | 같은 문서 §10 이 정정(실제 13종). 원문은 기록 보존을 위해 유지 |
| `functions/*.md` | 기능 기획서들이 Step 1~7 구조 변경 이전 기술일 가능성 | **미점검** |

### 4.5 §4A 두 벌 — **「두 벌」이라는 표현이 부정확하다** (2026-09-04)

「두 벌」은 대응 절이 1:1 이라는 뜻으로 읽힌다. **실제로는 절 번호가 어긋난 채 겹친다.**
...
**같은 번호가 서로 다른 절을 가리킨다.** 첫 벌의 `4A.5`(파일 내 검색)와 `4A.6`(`EditorViewRegistry`)는
**두 번째 벌에 대응 절이 아예 없다.**
